...
@section('title',"Admin Dashboard | Create Vehicle Assignment")
...
<section class="pt-3">
    <div class="card">
        <div class="card-header bg-info">
            <div class="card-title">Create New Vehicle Assignment</div>
        </div>
        <form action="{{ route('admin.vehicle_assignments.store') }}" method="POST">
            @csrf
                ...
                <div class="row py-2">
                    <div class="col-12 col-md-12 m-auto card p-5">
                        ...
                        <div class="form-group">
                            ...
                            <select name="vehicle_id" id="vehicle_id" class="form-control @error('vehicle_id') is-invalid @enderror">
                                ...
                                @foreach($vehicles as $vehicle)
                                    <option value="{{ $vehicle->id }}" {{ old('vehicle_id') == $vehicle->id ? 'selected' : '' }}>{{ $vehicle->name }} ({{ $vehicle->plate_number }})</option>
                                @endforeach
                            </select>
                            @error('vehicle_id')
                        ...
                        <div class="form-group">
                            ...
                            <select name="farmer_id" id="farmer_id" class="form-control @error('farmer_id') is-invalid @enderror">
                                ...
                                @foreach($farmers as $farmer)
                                    <option value="{{ $farmer->id }}" {{ old('farmer_id') == $farmer->id ? 'selected' : '' }}>{{ $farmer->name }} ({{ $farmer->email }})</option>
                                @endforeach
                            </select>
                            @error('farmer_id')
                            <p class="text-danger">{{ $message }}</p>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control @error('delivery_location') is-invalid @enderror" placeholder="Delivery Location" name="delivery_location" value="{{ old('delivery_location') }}">
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control @error('status') is-invalid @enderror">
                                ...
                                <option value="2" {{ old('status') == '2' ? 'selected' : '' }}>On the Way</option>
                                <option value="3" {{ old('status') == '3' ? 'selected' : '' }}>Delivered</option>
                            </select>
                            @error('status')
                            <p class="text-danger">{{ $message }}</p>
                            @enderror
            ...
            <div class="card-footer text-right">
                <input type="submit" class="btn btn-success mt-2" value="Save Assignment">
            </div>
        </form>
    </div>
</section>
